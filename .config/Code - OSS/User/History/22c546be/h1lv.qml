...
Item{
    ...
    property var activePlayer: {
        if(players.length > 0){
            for(let plap of players){
                if(plap.playBackState == "Playing") return plap;
            }
            for(let plapp of players){
                if(plapp.playBackState == "Paused") return plapp;
            }
        }
        return null
    }
    ...
    onPlayersChanged: {
        ...
        for(let pla of root.players){
            toPrint += `${pla.identity}: ${pla.playBackState}, `
        }
