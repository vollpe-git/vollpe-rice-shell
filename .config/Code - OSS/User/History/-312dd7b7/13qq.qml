...
Singleton {
    ...
    function setVolume(newVolume: real): void {
        if (sink?.ready && sink?.audio) {
            ...
            sink.audio.volume = Math.max(0, Math.min(1, newVolume)).toFixed(2);
        } else {
            console.log("error getting sink or sink audio");
            console.log(`is sink ready: ${sink?.ready}, sink: ${sink}, audio: ${sink?.audio}`)
        }
    }

    function incrementVolume(amount: real): void {
        setVolume(volume + (amount || root.step));
    }
    ...
    function incrementSourceVolume(amount: real): void {
        setSourceVolume(sourceVolume + (amount || root.step));
    }
    ...
    function toggleSinkMute(){
        if (root.sink){
            ...
            console.log("muted sink");
        } else console.log("sink is null")
    }
    ...
    PwObjectTracker {
        objects: [Pipewire.defaultAudioSink, Pipewire.defaultAudioSource, ...root.sinks, ...root.sources, ...root.streams]
    }
